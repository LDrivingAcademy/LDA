...
function FallbackMap({ progress }: { progress: number }) {
  ...
  return (
    <div className="relative h-[360px] overflow-hidden rounded border border-zinc-800 bg-zinc-950">
      ...
      <div className="absolute left-4 top-4 rounded bg-white px-3 py-2 text-sm font-black text-ink shadow-sm">
        Demo live map
      </div>
      <div className="absolute bottom-4 left-4 right-4 rounded border border-zinc-800 bg-ink/90 p-3 text-white backdrop-blur">
        <div className="flex items-center gap-2 text-sm font-bold">
          <RadioTower size={16} className="text-brand" /> Live location updates every few seconds
        </div>
      </div>
    </div>
  );
...
export function LiveLessonMap() {
  ...
  useEffect(() => {
    const interval = window.setInterval(() => {
      setProgress((current) => (current >= 0.92 ? 0.18 : current + 0.035));
    }, 2500);
        ...
        <div className="rounded border border-zinc-800 bg-zinc-950 p-4 shadow-2xl">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-3">
            ...
            <div className="rounded border border-zinc-800 bg-ink p-3">
              ...
              <div className="mt-1 font-black">2.5 sec</div>
            </div>
          </div>
        </div>
      </div>
