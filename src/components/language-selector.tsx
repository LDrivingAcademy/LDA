...
const languageOptions = [
  { code: "device", label: "Use device region" },
...
export function LanguageSelector() {
  const [deviceLanguage, setDeviceLanguage] = useState("en-GB");
  const [selected, setSelected] = useState("device");
  const [customLanguage, setCustomLanguage] = useState("");

  useEffect(() => {
    const stored = window.localStorage.getItem("lda-language");
    const detected = navigator.languages?.[0] || navigator.language || "en-GB";
    setDeviceLanguage(detected);
    setSelected(stored || "device");
  }, []);

  useEffect(() => {
    const activeLanguage = selected === "device" ? deviceLanguage : selected === "custom" ? customLanguage : selected;
    if (!activeLanguage) {
      return;
    }
  ...
  }, [customLanguage, deviceLanguage, selected]);

  const deviceLabel = useMemo(() => {
    try {
      const displayNames = new Intl.DisplayNames([deviceLanguage], { type: "language" });
      const name = displayNames.of(deviceLanguage.split("-")[0]);
      return name ? `${name} (${deviceLanguage})` : deviceLanguage;
    } catch {
  ...
  return (
    <details className="group relative">
      <summary className="inline-flex cursor-pointer list-none items-center gap-2 text-sm font-black hover:text-zinc-300 [&::-webkit-details-marker]:hidden">
        <Globe2 size={17} /> {labelFor(selected === "device" ? deviceLanguage : selected)} <ChevronDown size={14} />
      </summary>
      <div className="absolute right-0 top-full z-50 mt-3 w-80 rounded border border-zinc-800 bg-zinc-950 p-4 text-white shadow-2xl">
        <label className="block text-xs font-black uppercase text-zinc-400">Page language</label>
        <select
          value={selected}
        ...
        {selected === "custom" ? (
          <input
            ...
            placeholder="Example: en-GB, ar-EG, es-419"
            className="mt-3 w-full rounded border border-zinc-700 bg-black px-3 py-3 text-sm font-bold text-white"
          />
        ) : null}
        <p className="mt-3 text-xs leading-5 text-zinc-400">
          Device region detected as {deviceLabel}. Translation provider integration can be connected later for full live copy translation.
        </p>
      </div>
    </details>
  );
