...
import { instructorSteps } from "@/lib/marketplace-content";
            ...
            <div className="grid gap-8 py-10 lg:grid-cols-[1fr_460px]">
              <div>
                ...
                <div className="mt-8 flex flex-wrap gap-3">
                  <Link href="/auth/login?role=instructor" className="inline-flex items-center gap-2 rounded bg-brand px-5 py-3 text-sm font-black text-white hover:bg-brand-strong">
                    Start instructor sign up <ArrowRight size={16} />
                  </Link>
                  <Link href="/terms" className="inline-flex items-center gap-2 rounded border border-zinc-700 bg-white px-5 py-3 text-sm font-black text-ink hover:bg-zinc-100">
              ...
              <div className="rounded border border-zinc-800 bg-white p-5 text-foreground shadow-2xl">
                ...
                <div className="mt-4 grid gap-3">
                  {instructorSteps.map((step, index) => (
                    <div key={step} className="flex gap-3 rounded border border-border bg-background p-3">
        ...
        <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
          <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
            {onboardingBlocks.map((block) => {
              ...
              return (
                <article key={block.title} className="rounded border border-border bg-white p-5 shadow-sm">
          ...
          <div className="mx-auto grid max-w-7xl gap-6 px-4 py-10 sm:px-6 lg:grid-cols-[360px_1fr] lg:px-8">
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              {["Verification pending", "Approved profile", "Live availability"].map((item) => (
                <div key={item} className="rounded border border-border bg-card p-4">
