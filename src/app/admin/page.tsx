...
export default async function AdminPage() {
  ...
  const [users, learners, instructors, activeInstructors, bookings, paymentCount, reviewCount, { data: payments }, { data: reviews }, { data: recentBookings }] = await Promise.all([
    ...
    supabase.from("payments").select("gross_amount_pence,platform_fee_pence,status,created_at").limit(5000),
  ...
  const revenueMonth = paidPayments.filter((payment) => new Date(payment.created_at) >= monthStart).reduce((sum, payment) => sum + payment.platform_fee_pence, 0);
  const revenueYtd = paidPayments.filter((payment) => new Date(payment.created_at) >= yearStart).reduce((sum, payment) => sum + payment.platform_fee_pence, 0);
  const grossLessonValue = paidPayments.reduce((sum, payment) => sum + payment.gross_amount_pence, 0);
  const avgInstructorRating = reviews?.length ? (reviews.reduce((sum, review) => sum + review.rating, 0) / reviews.length).toFixed(1) : "0.0";
  ...
  const liveKpis = [
    ...
    { label: "Revenue this month", value: formatMoney(revenueMonth), detail: "Platform commission this month", icon: BadgePoundSterling },
    { label: "Revenue YTD", value: formatMoney(revenueYtd), detail: "Calendar-year platform commission", icon: BadgePoundSterling },
    { label: "Gross lesson value", value: formatMoney(grossLessonValue), detail: "Learner spend before commission", icon: BadgePoundSterling },
    { label: "Instructor reviews", value: reviewCount, detail: `${avgInstructorRating} average instructor rating`, icon: Star }
  ];

  return (
    <main className="min-h-screen bg-background">
      <header className="border-b border-border bg-ink px-4 py-8 text-white">
        <section className="mx-auto max-w-7xl">
          ...
          <p className="mt-3 max-w-3xl text-sm leading-6 text-zinc-300">Separate admin area for platform health: learners, instructors, bookings, payments, refunds, disputes, revenue, payouts, cancellation rate, and instructor reviews.</p>
        </section>
      </header>
          ...
          <section className="rounded border border-border bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 overflow-hidden rounded border border-border">
              <div className="grid grid-cols-4 bg-ink px-4 py-3 text-xs font-black uppercase text-white">
                <div>Status</div><div>Payment</div><div>Lesson value</div><div>Created</div>
              </div>
              {(recentBookings ?? []).map((booking) => (
                <div key={`${booking.created_at}-${booking.lesson_price_pence}`} className="grid grid-cols-4 border-t border-border px-4 py-3 text-sm">
              ...
              {recentBookings?.length ? null : <div className="px-4 py-6 text-sm text-muted">No live bookings yet. Demo KPI examples are shown alongside live counters until real payments arrive.</div>}
            </div>
          </section>

          <aside className="rounded border border-border bg-ink p-5 text-white shadow-sm">
